  ...
  return (
    <div className="  flex justify-center items-center">
      <div className="container w-[80%] bg-slate-100 min-h-[90vh]   rounded-lg p-6">
        <div className="bg-slate-100 flex gap-2">
          <Link
            to="/"
          ...
          <button className="bg-blue-400 rounded p-1">Rikiuoti A-Z</button>
          <button className="bg-blue-400 rounded p-1">Rikiuoti Z-A</button>

          <button className="bg-blue-400 rounded p-1">
            Filtruoti pagal procesorių
          </button>
          <button className="bg-blue-400 rounded p-1">
            Filtruoti pagal vaizdo plokštę
          </button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
          {" "}
          {pcList.map((pc) => (
            <MyPcPost pc={pc} key={pc.id} />
          ))}
        </div>
      </div>
    </div>
