...
// Function to generate options for category filter dropdown
...
function applyFilters() {
  ...
  const categoryFilter = document
    ...
    .value.toLowerCase();
  const priceMinFilter = parseFloat(
    document.getElementById("priceMinFilter").value
  );
  const priceMaxFilter = parseFloat(
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const priceMatch =
      (!isNaN(priceMinFilter) ? product.price >= priceMinFilter : true) &&
      (!isNaN(priceMaxFilter) ? product.price <= priceMaxFilter : true);
...
function renderProducts(filteredProducts) {
  ...
  for (let product of filteredProducts) {
    // ... (rest of your existing code to generate product HTML)
    dynamicHTML += `<!-- ... -->`;
  }
  const productList = document.getElementById("productList");
  productList.innerHTML = dynamicHTML;
}
...
for (let product of products) {
  if (product.stock < 20) stockWarning = "red";
  ...
  dynamicHTML += `
    <div class="row shadow bg-white border border-dark rounded gap-3 mx-1 mb-5 mt-2">
    ...
    product.stock
      ...
      <h2 class="mt-2 ">$${product.discountedPrice}</h2>
      <h3 class="text-secondary mt-1"><del>$${product.price}</del></h3>
      </div>
      <div class="d-grid mx-auto">
      <button class="p-1 mb-2 bg-warning text-dark">Add to Cart</button>
      </div>
      </div>
    </div>`;
}

const tbody = document.querySelector("#prodList");
